...
4. applies one `XXPlusYYGate` between boundary \(B\) and its local ancilla,
   with angle \(2\sqrt{2\gamma\,dt}\); and
5. resets only the selected boundary ancilla to \(|0\rangle\).
...
\[
N\text{ system qubits}+2\text{ boundary-local ancillas}=N+2\text{ qubits}.
\]

There is no mid-circuit measurement and no classical feed-forward. Reset is
still required because it implements tracing out the selected dilation
ancilla and prepares it for its next use. The logical wire order is

\[
a_R,\;s_{N-1},\ldots,s_0,\;a_L,
\]

so both possible jump gates are local at the logical level. Only one ancilla
participates in any sampled substep; the other remains idle in \(|0\rangle\).
...
also uses \(N+2\) qubits. The randomized circuit nevertheless eliminates the
mid-circuit measurement and conditional second jump. The second local
ancilla trades one idle hardware qubit for avoiding repeated routing of a
single reset ancilla between opposite chain ends. The actual
post-transpilation saving remains backend- and layout-dependent, so the
script records both logical and compiled operation counts and depths.
...
backend needs at least \(N+2\) qubits and reset support; unlike Experiment 4,
...
identifies every logical wire as a chain site or one of the two local boundary
ancillas. Matplotlib renders the topology and labels directly for consistent
...
The checked-in result artifacts in this section were generated by the
earlier one-ancilla circuit layout and are retained unchanged for provenance.
The current two-boundary-ancilla code deliberately treats those JSON files as
incompatible archives; a new run must use a fresh output directory (or the
legacy artifacts must first be archived manually). No existing result or
figure is migrated automatically.
